-- FarmaCapital — Pedido online: permitir medicamentos con receta.
-- La ficha ya dice «Se solicitará al entregar». Controlados siguen bloqueados.
-- Base: rpc_cliente_pedido_online_stock_efectivo.sql
-- Ejecutar en Supabase → SQL Editor → Run (después de backup).
...
begin
  ...
  loop
    ...
    if coalesce(v_prod.precio, 0) <= 0.01 then
