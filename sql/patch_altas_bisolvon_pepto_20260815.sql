-- Bisolvon Adulto 120 ml + Pepto-Bismol 118 ml (ticket FL-080826 / empaque)
...
-- ── 2) Pepto-Bismol Suspensión · 020800753067 (UPC; OCR ticket 0208007530671) ──
DO $$
DECLARE v_pid bigint; v_lid bigint;
BEGIN
  SELECT id INTO v_pid FROM public.productos
  ...
  IF v_pid IS NULL THEN
    ...
    FROM public.create_producto_with_lote(
      jsonb_build_object(
        ...
        'categoria', 'Digestivo',
        'tipo', 'marca',
        'descripcion', 'Pepto-Bismol subsalicilato 118 ml — P&G UPC 020800753067',
        'costo', 96.73,
        'precio', 150.00,
        'stock_minimo', 2,
        'activo', true,
...
WHERE p.codigo_barras IN ('7501037907117', '020800753067', '0208007530671')
ORDER BY p.nombre;
