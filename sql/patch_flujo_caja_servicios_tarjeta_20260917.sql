-- FarmaCapital — recargas: efectivo al cajón, tarjeta al corte de tarjeta.
-- 2026-09-17. Idempotente. Requiere patch_finanzas_piso_desde_sesiones_20260904.sql.
--
-- admin_flujo_caja_bundle sumaba todo pagos_servicio.total_cobrado
-- en cajon_cobrado_servicios. Si el cliente pagó con tarjeta, ese
-- dinero no entró al cajón. Aquí se parte:
--   cajon_cobrado_servicios   = solo efectivo
--   tarjeta_cobrada_servicios = solo tarjeta
-- El corte (reconcile_shift_cash) ya lo hacía.
...
begin
  ...
  select
    coalesce(sum(ps.total_cobrado) filter (where ps.metodo_pago = 'efectivo'), 0),
    coalesce(sum(ps.total_cobrado) filter (where ps.metodo_pago = 'tarjeta'), 0),
  ...
  into v_cub_cob, v_cub_tar, v_cub_liq, v_cub_comp, v_cub_com
  from public.pagos_servicio ps
  where (ps.created_at at time zone 'America/Mexico_City')::date >= v_desde
    and (ps.created_at at time zone 'America/Mexico_City')::date <= v_hasta;
  ...
  v_alertas := jsonb_build_array(
    ...
    jsonb_build_object(
      ...
      'nivel', 'info',
      'texto',
        'Dos cubetas: el efectivo de recargas ya va en el corte (cajón); la tarjeta va al corte de tarjeta. '
  ...
  return jsonb_build_object(
    ...
    'cubetas', jsonb_build_object(
      ...
      'tarjeta_cobrada_servicios', round(v_cub_tar, 2),
