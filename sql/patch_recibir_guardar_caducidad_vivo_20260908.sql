-- Recibir: "solo se edita una recepcion en borrador"
--
-- Recibir lista tickets vivos (borrador / pendiente_alta / pendiente_caducidad)
-- pero recepcion_confirmar_item solo aceptaba borrador. City Mark 20260905
-- (Neutrogena 7891010245160, GUM, Adidas…) se ve en la cola, pides MMAA
...
grant execute on function public.recepcion_agregar_item(uuid, bigint, text, integer, date, numeric)
  to anon, authenticated;
grant execute on function public.recepcion_quitar_item(uuid, bigint)
  to anon, authenticated;
