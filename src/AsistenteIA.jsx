...
const CHIPS = [
  ...
  "¿Cuáles son los precios de Similares hoy?",
  "¿Qué antibióticos requieren bitácora COFEPRIS?",
  "Sugiéreme productos para agregar al catálogo",
...
export default function AsistenteIA() {
  ...
  const sendMessage = async (text) => {
    ...
    try {
      ...
      const reply = data?.candidates?.[0]?.content?.parts?.[0]?.text || "No pude obtener respuesta.";
      setMessages(m=>[...m,{role:"assistant",content:reply}]);
    } catch(err) {
      setMessages(m=>[...m,{role:"assistant",content:`Error de conexión: ${err.message}`}]);
    }
    setLoading(false);
  };
  ...
  if (!apiKey) return (
    <div style={{padding:24,background:C.bg,minHeight:"100dvh",fontFamily:"'Plus Jakarta Sans',sans-serif",display:"flex",alignItems:"center",justifyContent:"center",boxSizing:"border-box",touchAction:"pan-y"}}>
      ...
      <div style={{display:"flex",gap:8,padding:"10px 16px",overflowX:"auto",borderBottom:`1px solid ${C.border}`,flexShrink:0,WebkitOverflowScrolling:"touch"}}>
        {CHIPS.map((chip,i)=>(
          <button key={i} onClick={()=>sendMessage(chip)} disabled={loading} style={{
            ...
            whiteSpace:"nowrap",flexShrink:0,opacity:loading?.6:1,
          }}>{chip}</button>
        ))}
      </div>
        ...
        {loading&&(
          <div style={{display:"flex",alignItems:"center",gap:8}}>
            ...
            <div style={{background:C.card,border:`1px solid ${C.border}`,borderRadius:"14px 14px 14px 4px",padding:"12px 16px",display:"flex",gap:5,alignItems:"center"}}>
              {[0,1,2].map(j=>(
                <div key={j} style={{width:7,height:7,borderRadius:"50%",background:C.blue,opacity:.7}}/>
              ))}
            </div>
          </div>
        )}
      ...
      <div style={{
        ...
        <div style={{display:"flex",gap:10,alignItems:"flex-end"}}>
          ...
          <button onClick={()=>sendMessage()} disabled={!input.trim()||loading} style={{
            ...
            opacity:!input.trim()||loading?.5:1,transition:"all .2s",
          }}>{loading?"…":"Enviar"}</button>
        </div>
        <div style={{color:C.textDim,fontSize:10,marginTop:6,textAlign:"right"}}>Enter para enviar · Shift+Enter nueva línea</div>
      </div>
    </div>
  );
}
