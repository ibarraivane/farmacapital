...
it("no deja que ProductCard (width:100% inline) estire la tarjeta a todo el ancho", () => {
  ...
  const { container } = render(
    <>
      <ProductosStripStyles />
      <RecompraStrip title="Comprar de nuevo">
...
it("lleva flechas izquierda y derecha aunque no haya overflow", () => {
  render(
    <RecompraStrip title="Sugerido para ti">
      <div>Uno</div>
    </RecompraStrip>
  );
  expect(screen.getByLabelText("Productos anteriores")).toBeInTheDocument();
  expect(screen.getByLabelText("Siguientes productos")).toBeInTheDocument();
});
