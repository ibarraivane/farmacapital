...
test("el carrusel mantiene el alto y Cotizar no hereda el fondo del navegador", () => {
  ...
  expect(css).toMatch(/\.fc-studio-icono\{height:var\(--fc-studio-media-h\);max-height:var\(--fc-studio-media-h\)[^}]*flex:none/);
  ...
  expect(css).toMatch(/\.fc-studio-stack\{display:grid/);
  expect(css).toMatch(/\.fc-studio-slide\{[^}]*grid-area:1\/1/);
  expect(css).toMatch(/\.fc-studio-slide\.is-active\{[^}]*opacity:1/);
  expect(css).toMatch(/\.fc-packshots\{[^}]*max-height:var\(--fc-studio-media-h\)/);
  expect(css).toMatch(/\.fc-studio \*\{overflow-anchor:none\}/);
});
