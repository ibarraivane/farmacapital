...
function TarjetaMarca({ marca, onSelect, onAdd, estadoStock, fotoDe }) {
  const C = C_LIGHT;
  const conFoto = marca.opciones.find(({ p }) => p.imagen_url || p.imagen_mobile_url || fotoDe?.(p.id));
  const foto = conFoto ? (conFoto.p.imagen_url || conFoto.p.imagen_mobile_url || fotoDe?.(conFoto.p.id)) : "";
  ...
  return (
    <div style={{ border: `1px solid ${C_LIGHT.border}`, borderRadius: 14, background: C_LIGHT.bg, padding: 10, marginBottom: 10 }}>
      ...
      <div className="pos-related-products-grid" style={{ display: "grid", gridTemplateColumns: "repeat(2, minmax(0, 1fr))", gap: 8, marginTop: 8 }}>
        {marcas.map((m) => (
          <TarjetaMarca key={m.marca} marca={m} onSelect={onSelect} onAdd={onAdd} estadoStock={estadoStock} fotoDe={fotoDe} />
        ))}
      </div>
