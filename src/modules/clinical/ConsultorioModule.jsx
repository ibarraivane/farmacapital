...
import { imprimirRecetaCarta } from "../../utils/recetaPrint";
...
function EnConsulta() {
  ...
  const imprimirReceta = () => {
    ...
    imprimirRecetaCarta({
      cita: citaActual,
      medico: { nombre: "Dra. Lourdes Lucio Falcón", especialidad: "Medicina general" },
      diagnostico,
      notas: notasMedico,
      alergias,
      medicamentos: medicamentos.filter((m) => String(m.medicamento || "").trim()),
    });
  };
        ...
        <div style={{display:"flex",flexDirection:"column",gap:14}}>
          ...
          <div style={{display:"flex",gap:10}}>
            ...
            <button onClick={imprimirReceta} disabled={!diagnostico.trim()} style={{
              padding:"13px", fontSize:14, flex:1, borderRadius:8, border:"1px solid #0D1B2A",
              background:"#eff6ff", color:"#0D1B2A", cursor:!diagnostico.trim()?"not-allowed":"pointer",
