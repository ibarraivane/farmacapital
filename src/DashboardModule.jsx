...
export default function DashboardModule({ usuario, setPage }) {
  ...
  const fetchAll = useCallback(async () => {
    ...
    const [
      ...
      {data:citasHoy},{data:pedOnline},{data:pedMesTipo},
      {data:pedItems},{data:bajoStock},{data:porCaducar},
      {data:pedPend},{data:cortesConDif},
    ] = await Promise.all([
      ...
      supabase.from("citas").select("id").eq("estado","completada").gte("fecha",today.start).lte("fecha",today.end),
      supabase.from("pedidos").select("id").eq("estado","pendiente").eq("tipo","online"),
    ...
    setData({
      ventasHoy,ventasSemana,ventasMes,ventasMesAnt,crecimiento,ticketProm,consultasHoy:(citasHoy||[]).length,onlinePend:(pedOnline||[]).length,
      ...
      alertas:{bajoStock:(bajoStock||[]).length,bajoStockNombres:(bajoStock||[]).map(p=>p.nombre).slice(0,3),porCaducar:(porCaducar||[]).length,sinAtender:(pedPend||[]).length,cortesConDif:(cortesConDif||[]).length},
    });
    setLoading(false);
  }, []);
  ...
  const {ventasHoy,ventasSemana,ventasMes,ventasMesAnt,crecimiento,ticketProm,consultasHoy,onlinePend,recuperado,pctRecuperado,gananciaMes,paybackMeses,restante,fuentes,empleados,topProductos,alertas} = data;
  ...
  return (
    <div style={{padding:24,background:C.bg,minHeight:"100vh",fontFamily:"'Plus Jakarta Sans',sans-serif"}}>
      <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:24,flexWrap:"wrap",gap:12}}>
        <div>
          <h1 style={{margin:0,color:C.text,fontSize:20,fontWeight:800}}>◈ Dashboard</h1>
          <p style={{margin:"4px 0 0",color:C.textMid,fontSize:12,textTransform:"capitalize"}}>{fmtDate()}</p>
        </div>
        <div style={{display:"flex",gap:10,alignItems:"center"}}>
          <div style={{color:C.textMid,fontSize:12}}>Hola, <strong style={{color:C.text}}>{usuario?.nombre?.split(" ")[0]}</strong> 👋</div>
          <button onClick={fetchAll} style={{padding:"7px 14px",borderRadius:8,border:`1px solid ${C.border}`,background:"transparent",color:C.textMid,cursor:"pointer",fontWeight:700,fontSize:12}}>🔄 Actualizar</button>
        </div>
      </div>
      ...
      <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fill,minmax(160px,1fr))",gap:12,marginBottom:24}}>
        ...
        <KpiCard label="Consultas hoy"     value={consultasHoy}       col={C.green}  icon="♥"  sub="completadas"/>
