...
export default function DashboardModule({ usuario, setPage, showConfirm, initialTab }) {
  ...
  const fetchAll = useCallback(async () => {
    ...
    // Recargas y pagos de servicio del mes: misma atribución que Mi Día / comisiones.
    if (pagosSrvMesRes?.error) {
      ...
      const pagosMes = parseRpcJsonArray(pagosSrvMesRes?.data).filter(
        (r) => ymdMexico(r.created_at) >= inicioMesLocal && ymdMexico(r.created_at) <= hoyLocal
      );
      acumularServiciosEnMapaEmpleado(byEmp, pagosMes);
    }
