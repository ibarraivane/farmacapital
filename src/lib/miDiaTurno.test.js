...
describe("resolverTurnoMiDia", () => {
  const manana = new Date("2026-09-14T15:00:00.000Z"); // 09:00 CDMX
...
describe("resolverTurnosMetaHoy", () => {
  const manana = new Date("2026-09-14T15:00:00.000Z");
...
describe("resolverVentanaVentasMiDia", () => {
  const diaRango = {
    inicio: new Date(2026, 8, 14, 0, 0, 0),
    fin: new Date(2026, 8, 14, 23, 59, 59, 999),
  };

  test("caja abierta usa abierta_at → ahora", () => {
    const v = resolverVentanaVentasMiDia({
      sesionCaja: { abierta: true, abierta_at: "2026-09-14T14:05:00.000Z", turno: "matutino" },
      diaRango,
      turno: "vespertino",
    });
    expect(v.fuente).toBe("caja_abierta");
    expect(v.inicio).toBe("2026-09-14T14:05:00.000Z");
  ...
  test("sin caja: día completo aunque el perfil sea vespertino", () => {
    ...
    expect(new Date("2026-09-14T21:11:00.000Z").getTime()).toBeGreaterThanOrEqual(new Date(v.inicio).getTime());
    expect(new Date("2026-09-14T21:11:00.000Z").getTime()).toBeLessThanOrEqual(new Date(v.fin).getTime());
  });
});
