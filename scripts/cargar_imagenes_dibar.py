#!/usr/bin/env python3
"""Packshots Dibar confirmados en la etiqueta para SKUs sin foto.

No adivina ni reusa el frasco de 1 L en otra talla. La lista curada está en
sql/generated/fotos_dibar_confirmadas_20260830.csv.

Uso:
    python3 scripts/cargar_imagenes_dibar.py --check
    python3 scripts/cargar_imagenes_dibar.py --aplicar
...
CURADAS = ROOT / "sql/generated/fotos_dibar_confirmadas_20260830.csv"
SITIOS_OK = (
    "tienda.zafir.mx",
    "cdn.shopify.com",
    "pmvelasco.mx",
)
UA = "FarmaCapitalCatalog/1.0 (fotos dibar)"
...
def check():
    ...
    for row in filas:
        if not any(d in row["permalink"] for d in SITIOS_OK):
            fallos.append(f"{row['sku']}: permalink no es distribuidor verificado")
            continue
        if not any(d in row["foto"] for d in SITIOS_OK):
            fallos.append(f"{row['sku']}: foto no es CDN del distribuidor")
            continue
    ...
    print(f"{len(filas)} packshots Dibar vivos")
...
def aplicar():
    ...
    if not base or not key:
        raise SystemExit(
            ...
            "Mientras tanto corre sql/patch_fotos_dibar_20260830.sql "
...
def main():
    ...
    parser.add_argument("--check", action="store_true", help="GET a las URLs y valida magic bytes")
