...
`;
  ...
  fs.writeFileSync(
    ...
    `-- Este archivo junta las ${productos.length} filas y el editor se va de tiempo.\nselect 'Corre aplicar_01.sql a aplicar_10.sql, en orden.' as aviso;\n`,
  );
  const BLOQUES = 10;
  for (let i = 0; i < BLOQUES; i += 1) {
    const n = String(i + 1).padStart(2, "0");
    const filtro = `mod(abs(hashtext(t.sku)::bigint), ${BLOQUES}) = ${i}`;
    fs.writeFileSync(
      path.join(pegar, `aplicar_${n}.sql`),
      `-- Bloque ${i + 1} de ${BLOQUES}. Pasa una parte al inventario.
