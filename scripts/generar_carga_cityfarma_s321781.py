...
Panclasa), no del renglón del ticket. Sin lote ni MMAA: salen de la caja al escanear.
...
# Geslutin 200 mg C/15 — YZA EAN 7501871720620 + Fahorro + Asofarma
# Panclasa 80/80 mg C/20 — Fahorro/Farmamedical EAN 7501471800265 (Atlantis)
ROWS = [
    {
        "ean": "7501871720620",
...
if __name__ == "__main__":
    ...
    for x in ROWS:
        ...
        assert "GESLUTIN 200MG" not in x["nombre"].upper() or "progesterona" in x["nombre"].lower()
        assert not x["nombre"].startswith(x["snap"][:8]) or "Geslutin" in x["nombre"] or "Panclasa" in x["nombre"]
