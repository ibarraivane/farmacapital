...
/** Renglón para Recibir: no inventa caducidad, código ni lote. */
function renglonDesdeRaw(raw) {
  if (!raw || typeof raw !== 'object') return null;
  const codigo = String(
    raw.codigo_barras || raw.codigo || raw.ean || raw.upc || raw.barcode || ''
  ).replace(/\D/g, '').trim();
  const sku = String(raw.sku || raw.sku_farmacapital || '').trim();
  const nombre = String(
    raw.nombre || raw.descripcion || raw.descripcion_ticket || raw.name || ''
  ).trim();
  if (!nombre && !codigo && !sku) return null;
  const cantidad = parseEntero(raw.cantidad ?? raw.qty ?? raw.cant ?? raw.stock, 1);
  const costoRaw = raw.costo ?? raw.precio_unitario ?? raw.precio ?? raw.precioUnitario;
  const costo = costoRaw == null || costoRaw === '' ? null : parseNumero(costoRaw, null);
  const lote = String(raw.lote || raw.numero_lote || '').trim();
  return {
    codigo: codigo || null,
    sku: sku || null,
...
const CLAUDE_TICKET_PROMPT = `Eres un extractor de tickets de compra farmacéutica en México.
Devuelve ÚNICAMENTE JSON válido (sin markdown):
{
  "proveedor": "nombre si aparece, si no null",
  "folio": "folio o número de ticket si aparece, si no null",
  "total": 2570.99,
  "productos": [
    {
...
Reglas:
- codigo = código de barras si está impreso (8-14 dígitos); si no, null. NUNCA inventes un código.
- caducidad: SOLO si está impresa en el ticket; si no, null. NUNCA inventes una fecha.
- lote: SOLO si está impreso; si no, null.
- precio = precio unitario MXN; cantidad = piezas.
- Incluye TODOS los renglones. Si no hay productos: {"productos":[]}`;
...
async function extraerConClaude(apiKey, pdfBase64, options = {}) {
  const prompt = options.prompt || (options.ticketCompleto ? CLAUDE_TICKET_PROMPT : CLAUDE_EXTRACT_PROMPT);
  ...
  if (options.ticketCompleto) return parsed && typeof parsed === 'object' ? parsed : { productos: [] };
